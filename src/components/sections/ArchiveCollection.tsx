...
import { archiveTracks, type AudioTrack } from '@/data/phase3';
...
export function ArchiveCollection() {
  ...
  const results = useMemo(() => {
    ...
    let list = archiveTracks.filter((track) => !term || [track.title, track.artist, track.location, track.language, track.tradition].some((v) => v.toLowerCase().includes(term)));
  ...
  const random = () => { const pool = results.length ? results : archiveTracks; playTrack(pool[Math.floor(Math.random() * pool.length)]); };

  return (
    <section id="archive" data-theme="archive-paper" className="bg-[var(--page-bg)] px-4 py-20 text-[var(--text-primary)] sm:px-6 lg:px-10 lg:py-28">
      <div className="mx-auto max-w-[1400px]">
        <header><p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-[var(--accent-primary)]">Fresh from the Archive</p><h2 className="mt-4 font-devanagari text-3xl sm:text-4xl lg:text-5xl">नई मिलीं, पुरानी धुनें</h2><p className="mt-3 max-w-2xl text-sm text-[var(--text-secondary)] sm:text-base">Discover demonstration recordings recently contributed by artists, families and local communities.</p></header>
...
function FeaturedTrack({ track, onPlay, saved, onSave, onShare }: TrackProps) { return <article className="relative min-h-[480px] overflow-hidden rounded-[2rem]"><img src={track.artwork} alt="" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#241719] via-[#241719]/30 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-7 text-white sm:p-9"><p className="text-[10px] uppercase tracking-[0.22em] text-[#F0B23D]">Featured recording · Demonstration</p><h3 className="mt-2 font-devanagari text-3xl">{track.title}</h3><p className="mt-1 text-sm text-[#eadac8]">{track.artist}</p><p className="mt-3 flex items-center gap-2 text-xs text-[#eadac8]"><MapPin className="h-4 w-4" />{track.location} · {track.language}</p><div className="mt-6 flex gap-2"><PlayButton track={track} onPlay={onPlay} /><IconButton label={saved ? 'Remove saved track' : 'Save track'} onClick={onSave}><Bookmark fill={saved ? 'currentColor' : 'none'} /></IconButton><IconButton label="Share track" onClick={onShare}><Share2 /></IconButton></div></div></article>; }
