...
import type { ApprovedAudioTrack } from '@/contexts/PublicSongsContext';

export interface SearchItem { id: string; group: 'Songs' | 'Places' | 'Pages'; title: string; subtitle: string; terms: string; href: string; }
export function buildSearchIndex(regions: IndiaRegion[], tunes: Array<DemoTune | ApprovedAudioTrack>): SearchItem[] {
  return [
    ...tunes.map((tune) => ({ id: tune.id, group: 'Songs' as const, title: tune.title, subtitle: `${tune.artist} · ${tune.region} · ${tune.language}`, terms: [tune.title, tune.artist, tune.region, tune.language, 'genre' in tune ? tune.genre : '', tune.tradition].join(' ').toLowerCase(), href: `/states/${tune.stateSlug}` })),
    ...
    { id: 'page-upload', group: 'Pages', title: 'Upload Folk Music', subtitle: 'Prepare a contribution', terms: 'upload contribute preserve song recording अपलोड लोक संगीत', href: '/upload' },
  ];
}
